"""
Test the table encryption
"""

import pytest
from src.crypto.table import TableCipher, get_table, maketrans, translate
from src.common import compat_ord
...
def test_table():
    ...
    for i in range(0, 256):
        assert (target1[0][i] == compat_ord(encrypt_table[i]))
        assert (target1[1][i] == compat_ord(decrypt_table[i]))

    encrypt_table = b''.join(get_table(b'barfoo!'))
    decrypt_table = maketrans(encrypt_table, maketrans(b'', b''))

    for i in range(0, 256):
        assert (target2[0][i] == compat_ord(encrypt_table[i]))
        assert (target2[1][i] == compat_ord(decrypt_table[i]))
